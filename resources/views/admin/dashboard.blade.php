        ...
        <tbody>
            @forelse ($peminjamanTerkini as $index => $p)
                <tr>
                    ...
                    <td>
                        {{-- [PERBAIKAN] Membungkus semua form dalam satu div --}}
                        <div class="action-buttons">
                            @if ($p->status === 'pending')
                                ...
                                <form action="{{ route('admin.peminjaman.reject', $p->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn-reject" title="Tolak"><i class="fas fa-times"></i></button>
                                </form>

                            @elseif ($p->status === 'disetujui')
                                <form action="{{ route('admin.peminjaman.complete', $p->id) }}" method="POST" onsubmit="return confirm('Selesaikan peminjaman ini?')">
                                    @csrf
                                    <button type="submit" class="btn-complete" title="Selesaikan">Selesaikan</button>
                                </form>
                            ...
                            @elseif ($p->status === 'menunggu_validasi')
                                <form action="{{ route('admin.peminjaman.validate', $p->id) }}" method="POST" onsubmit="return confirm('Validasi peminjaman ini sebagai selesai?')">
                                    @csrf
                                    <button type="submit" class="btn-validate" title="Validasi Selesai">Validasi</button>
                                </form>
...
{{-- [PERBAIKAN] Tambahan CSS untuk menata tombol --}}
<style>
    .action-buttons {
        display: flex;
